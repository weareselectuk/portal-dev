...
if($this->is_administrator($current_user)){
    ...
    if( (in_array( $ticket->cat_id, $supervisor_categories ) || in_array( $current_user->ID, $assigned_agents )) && $agent_settings['supervisor_allow_delete_ticket'] ){
    ...
    if( in_array( $current_user->ID, $assigned_agents ) && $agent_settings['agent_allow_delete_ticket'] ){
...
$flag = apply_filters( 'wpsp_ticket_cap_delete_ticket', $flag, $ticket );
