...
if(!isset($_REQUEST['com']) && !$_REQUEST['section']=='company-settings' && !isset($_REQUEST['edit'])){
    exit;
}

global $wpspcompany;

$company_id=sanitize_text_field($_REQUEST['com']);

$company=$wpspcompany->functions->get_company_by_id($company_id);
...
<div id="tab_container" style="clear: both;">
    ...
    <div class="wpsp_company_setting_div">
        ...
        <form method="post" action="" onsubmit="return validate_update_company_form(this);">

            <input type="hidden" name="action" value="update"/>
            <input type="hidden" name="update_setting" value="wpsp_update_company"/>
            <?php wp_nonce_field('wpbdp_tab_general_section_general'); ?>

            <h3><?php echo __('Update Company / Usergroup','wpsp-company') ?><a class="backtoconditionlist" href="<?php echo $section_href;?>"> Back To List</a></h3>
            <hr>
            <table class="form-table">

                <tr>
                    <th><?php _e('Name','wpsp-company');?>:</th>
                    <td><input type="text" id="wpsp_company_name" name="wpsp_company_name" value="<?php echo $company->name; ?>"></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <?php
                        $comp_users=$wpspcompany->functions->get_company_users($company_id);
                        ?>
                        <table id="wpsp_company_selected_users">
                            ...
                            <tbody>
                                <?php
                                    $company_supervisors=$wpspcompany->functions->get_company_supervisors($company_id);
                                    foreach ($comp_users as $user){
                                        $checked="";
                                        if(in_array($user->ID, $company_supervisors)){
                                            $checked="checked";
                                        }
                                        echo "<tr><td>" . $user->display_name . "</td><td><input type='checkbox' name='csupervisor[]' value='".$user->ID."' ".$checked.">".
                                                    "<input type='hidden' name='cuserid[]' value='".$user->ID."'/> </td>".
...
<script>
    ...
    function validate_update_company_form(obj){
        var error_flag = true;
