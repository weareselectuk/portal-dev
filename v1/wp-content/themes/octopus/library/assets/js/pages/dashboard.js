...
jQuery(function() {    
    ...
    jQuery('.daterange-ranges').daterangepicker(
        {
        ...
        function(start, end) {
            jQuery('.daterange-ranges span').html(start.format('MMMM D') + ' - ' + end.format('MMMM D'));
        }
    );

    jQuery('.daterange-ranges span').html(moment().subtract(29, 'days').format('MMMM D') + ' - ' + moment().format('MMMM D'));
    ...
    function trafficSources(element, height) {
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function (error, data) {
            ...
            d3Container
                .on("mousemove", function (d, i) {
                    ...
                    tooltip.style("top", (mousey - (jQuery('.d3-tip').outerHeight() / 2)) - 2 + "px") // Half tooltip height - half arrow width

                    // Move tooltip horizontally
                    if(mousex >= (jQuery(element).outerWidth() - jQuery('.d3-tip').outerWidth() - margin.right - (tooltipOffset * 2))) {
                        tooltip
                            .style("left", (mousex - jQuery('.d3-tip').outerWidth() - tooltipOffset) + "px") // Change tooltip direction from right to left to keep it inside graph area
                            .attr("class", "d3-tip w");
                    }
                    else {
                        tooltip
        ...
        jQuery(window).on('resize', resizeStream);

        // Call function on sidebar width change
        jQuery(document).on('click', '.sidebar-control', resizeStream);
    ...
    function appSalesLines(element, height) {
        ...
        // Menu
        var menu = jQuery("#select_date").multiselect({
        ...
        // Radios
        jQuery(".multiselect-container input").uniform({ radioClass: 'choice' });
        ...
        function redraw() { 
            ...
            // Call function on window resize
            jQuery(window).on('resize', appSalesResize);

            // Call function on sidebar width change
            jQuery(document).on('click', '.sidebar-control', appSalesResize);
    ...
    function salesHeatmap() {
        ...
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function(error, data) {
            ...
            // Call function on window resize
            jQuery(window).on('resize', resizeHeatmap);

            // Call function on sidebar width change
            jQuery(document).on('click', '.sidebar-control', resizeHeatmap);
    ...
    function monthlySalesArea(element, height, color) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (error, data) {
            ...
            // Call function on window resize
            jQuery(window).on('resize', monthlySalesAreaResize);

            // Call function on sidebar width change
            jQuery(document).on('click', '.sidebar-control', monthlySalesAreaResize);
    ...
    function messagesArea(element, height, color) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (error, data) {
            ...
            // Call function on window resize
            jQuery(window).on('resize', messagesAreaResize);

            // Call function on sidebar width change
            jQuery(document).on('click', '.sidebar-control', messagesAreaResize);
    ...
    function sparkline(element, chartType, qty, height, interpolation, duration, interval, color) {
        ...
        // Call function on window resize
        jQuery(window).on('resize', resizeSparklines);

        // Call function on sidebar width change
        jQuery(document).on('click', '.sidebar-control', resizeSparklines);
    ...
    function dailyRevenue(element, height) {
        ...
        // Call function on window resize
        jQuery(window).on('resize', revenueResize);

        // Call function on sidebar width change
        jQuery(document).on('click', '.sidebar-control', revenueResize);
    ...
    function progressMeter(element, width, height, color) {
        ...
        var d3Container = d3.select(element),
            ...
            twoPi = 2 * Math.PI,
            progress = jQuery(element).data('progress'),
            total = 100;
    ...
    function generateBarChart(element, barQty, height, animate, easing, duration, delay, color, tooltip) {
        ...
        // Call function on window resize
        jQuery(window).on('resize', barsResize);

        // Call function on sidebar width change
        jQuery(document).on('click', '.sidebar-control', barsResize);
    ...
    function bulletChart(element, height) {
            ...
            d3.bullet = function() {
                ...
                function bullet(g) {
                    g.each(function(d, i) {
                        ...
                        // Call function on window resize
                        jQuery(window).on('resize', resizeBulletsCore);

                        // Call function on sidebar width change
                        jQuery(document).on('click', '.sidebar-control', resizeBulletsCore);
        ...
        d3.json("assets/demo_data/dashboard/bullets.json", function(error, data) {
            ...
            // Call function on window resize
            jQuery(window).on('resize', bulletResize);

            // Call function on sidebar width change
            jQuery(document).on('click', '.sidebar-control', bulletResize);
    ...
    jQuery(".table tr").each(function (i) {
        ...
        // Title
        var $title = jQuery(this).find('.letter-icon-title'),
            letter = $title.eq(0).text().charAt(0).toUpperCase();

        // Icon
        var $icon = jQuery(this).find('.letter-icon');
