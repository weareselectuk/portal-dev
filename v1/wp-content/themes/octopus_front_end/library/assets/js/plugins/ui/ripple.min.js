...
jQuery(function(){
	...
	jQuery('.dp-item, .dp-nav, .sidebar-xs .sidebar-main .navigation > li > a').ripple({unbind: true});
	
	jQuery(document).on('click', '.sidebar-control', function() {
		if(jQuery('body').hasClass('sidebar-xs')) {
			jQuery('.sidebar-main .navigation > li > a').ripple({unbind: true});
		}
		else {
			jQuery('.sidebar-main .navigation > li > a').ripple({unbind: false});
		}
	});
});
